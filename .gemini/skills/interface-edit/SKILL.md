...
```powershell
powershell.exe -NoProfile -File ".gemini/skills/interface-edit/scripts/interface-edit.ps1" -CIPath '<path>' -Operation hide -Value '<cmd>'
```

### JSON mode

```powershell
powershell.exe -NoProfile -File ".gemini/skills/interface-edit/scripts/interface-edit.ps1" -CIPath '<path>' -DefinitionFile '<json>'
```
