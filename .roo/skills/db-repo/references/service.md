...
```powershell
powershell.exe -NoProfile -File ".roo/skills/db-repo/scripts/db-repo.ps1" -Command set-label -InfoBasePath "C:\Bases\MyDB" -Label "Релиз 1.2" -Version 120 -Comment "Передано в тест"
```
...
```powershell
powershell.exe -NoProfile -File ".roo/skills/db-repo/scripts/db-repo.ps1" -Command optimize -InfoBasePath "C:\Bases\MyDB"
```
...
```powershell
powershell.exe -NoProfile -File ".roo/skills/db-repo/scripts/db-repo.ps1" -Command clear-cache -InfoBasePath "C:\Bases\MyDB" -CacheScope local
```
