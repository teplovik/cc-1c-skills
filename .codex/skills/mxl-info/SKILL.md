...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".codex/skills/mxl-info/scripts/mxl-info.ps1" -TemplatePath "<путь>"
```

Или по имени обработки/макета:
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".codex/skills/mxl-info/scripts/mxl-info.ps1" -ProcessorName "<Имя>" -TemplateName "<Макет>" [-SrcDir "<каталог>"]
```
